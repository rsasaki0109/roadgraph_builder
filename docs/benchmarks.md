...
## v0.7.2-dev functional shortest_path planner cache (2026-04-23)
...
CI comparison mode (`--baseline docs/assets/benchmark_baseline_0.7.2-dev.json`)
fails with exit code 1 if any benchmark regresses by more than 200 % (i.e. runs
3× slower than baseline). `--output` writes the same JSON shape accepted by
`--baseline`; the script rejects using the same file for both flags so the
committed baseline is not overwritten by accident.
...
python scripts/run_benchmarks.py --baseline docs/assets/benchmark_baseline_0.7.2-dev.json

# Refresh a candidate baseline after an intentional performance change
python scripts/run_benchmarks.py --no-warmup --output /tmp/benchmark_baseline_candidate.json
```
