...
| `map_paris_grid.lanelet.osm` | Per-lane Lanelet2 OSM XML generated by `export_lanelet2_per_lane()` from `map_paris_grid.geojson` after `infer_lane_counts(base_lane_width_m=3.5)`. 1081 `type=lanelet` relations each with left/right boundary ways derived from the HD-lite envelopes. Autoware's `lanelet2_validation` can load this file. **It is HD-lite, not survey-grade**: lane widths are 3.5 m offsets from the centerline, lane counts default to 1 because the Paris grid has no real lane markings, and elevation is unknown (`ele` absent). Deployment on a real vehicle still requires calibrated sensors and cm-class QA. | **© OpenStreetMap contributors, ODbL 1.0** |
