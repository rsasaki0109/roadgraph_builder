...
| `map_paris_grid.lanelet.osm` | Per-lane Lanelet2 OSM XML generated by `export_lanelet2_per_lane()` from `map_paris_grid.geojson` after `infer_lane_counts(base_lane_width_m=3.5)`. 1081 `type=lanelet` relations each with left/right boundary ways derived from the HD-lite envelopes. Every lanelet now carries the Autoware-spec tags `one_way` (from OSM `oneway`), `participant:vehicle=yes`, `speed_limit=<N> km/h` (from OSM `maxspeed`), `name` (OSM street name), plus `type=lanelet` / `subtype=road` / `location=urban`. Autoware's `lanelet2_validation` can load this file, and the repo's own `validate-lanelet2-tags` reports `"result": "ok"` with 0 errors. **It is still HD-lite, not survey-grade**: lane widths are 3.5 m offsets from the centerline, lane counts default to 1 because the Paris grid has no real lane markings, and elevation is unknown (`ele` absent). Deployment on a real vehicle still requires calibrated sensors and cm-class QA. | **© OpenStreetMap contributors, ODbL 1.0** |
| `paris_grid_camera_detections.json` | Hand-authored synthetic detections (traffic lights, stop lines, crosswalks, speed limits) placed along the committed Paris TR route so the viewer can render regulatory overlays without real camera imagery. `scripts/refresh_docs_assets.py` pipes them through `apply_camera_detections_to_graph` and adds matching Point features to `map_paris_grid.geojson` during regeneration. | Repo license (synthetic, no real imagery) |
