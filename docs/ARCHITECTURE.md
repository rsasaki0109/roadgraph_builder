...
| `sanitize-lanelet2-autoware` | Lanelet2 OSM XML | conservative `lanelet2_map.osm` + optional `map_projector_info.yaml` for loader smoke tests | `io.export.lanelet2.sanitize_lanelet2_for_autoware` |
...
| `match-trajectory` | road_graph + trajectory CSV | per-sample snap JSON + coverage summary; `--hmm` uses along-edge transition distances; `--explain` adds edge-index/timing diagnostics | `routing.map_match.snap_trajectory_to_graph` / `routing.hmm_match.hmm_match_trajectory` |
