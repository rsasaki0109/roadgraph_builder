...
| `match-trajectory` | road_graph + trajectory CSV | per-sample snap JSON + coverage summary; `--explain` adds edge-index/timing diagnostics | `routing.map_match.snap_trajectory_to_graph` / `routing.hmm_match.hmm_match_trajectory` |
