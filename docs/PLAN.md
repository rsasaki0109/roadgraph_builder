...
- **latest Codex checks (2026-04-28/29 sessions):** `git diff --check` clean。
  ...
  は **14 passed**。`node --check docs/js/map_console.js` pass。
  `python3 -m pytest tests/test_viewer_js_dijkstra.py tests/test_route_geojson.py tests/test_reachability.py`
  は **14 passed**。`make viewer-smoke` は **1 passed / 671 deselected**。
  Autoware 実行確認では rootless ROS Jazzy overlay から
  `autoware_lanelet2_validation` を起動し、rich SF Lanelet2 OSM は validator
  exit 0 だが stock Lanelet2 strict load は custom `lane_change` /
  `lane_connection` / sample `traffic_light` regulatory_element で失敗。
  新規 `sanitize-lanelet2-autoware` で
  `/tmp/roadgraph_autoware_sf_map_compat/lanelet2_map.osm` を生成すると
  strict `lanelet2.io.load` は 3 458 lanelets / 33 594 points /
  7 632 linestrings / 238 regulatory_elements（全部 lanelet に attached）
  で成功し、Autoware validator は
  `starting validation` → `finished validation` の exit 0（RTPS SHM warning のみ）。
  改善後の sanitizer は missing `ele` を近傍の既存標高から埋め、lanelet
  geometry から `turn_direction` を推定（SF: straight 2 504 / right 512 /
  left 442）、sample traffic_light 238 件を Lanelet2 `traffic_light`
  regulatory_element（generated linestring + height + `traffic_light_id` +
  `light_bulbs`）として保持し、`--map-projector-info-yaml` で Autoware
  sidecar も出力する。
...
- **直近の sessions (2026-04-21〜2026-04-29) で landed / in-progress:**
  ...
  - **2026-04-28 Codex session:** §5b A を実行し、San Francisco North Beach /
    ...
    derived docs assets を commit 対象として追加。README / SHOWCASE / docs/index /
    CHANGELOG / ATTRIBUTION / PLAN の文言を SF + directed `lane_connection` 前提に更新。
  0. **2026-04-26 (this Claude session):** Lanelet2 Autoware-friendliness 3 連発。
     - `lane_connection` regulatory_element を **方向付きペア** 化。junction node ごとに incident
       lanelet を 1 本 bundle にしていた既存出力（`from_start` / `from_end` ロール）を廃して、
       `(predecessor, successor)` の lanelet 対ごとに 1 relation を emit する形へ refactor。
       `osm_oneway=yes` / `oneway=-1` を解釈し、reverse-only edge は逆向き successor に
       現れない。重複ブロックを `_emit_lane_connection_relations` ヘルパーに集約、
       `_flow_directions_for_edge` で oneway を扱う。Paris 707 → 1 705、Berlin 1 476 → 4 962、
       Tokyo 464 → 1 327 の lane_connection に増えた状態で `validate-lanelet2-tags` 引き続き ok。
     - `tests/test_lanelet_summary.py` を新規追加。`scripts/refresh_docs_assets.py` の private
       helper `_emit_lanelet_summary` を `importlib.util` で読み込み、(1) 自前合成 graph で
       生成した Lanelet2 OSM の counts と payload が JSON と一致すること、(2) 入力 OSM が
       無いとき summary を書かないこと、そして (3) **commit されている
       `docs/assets/map_*_lanelet_summary.json` が `docs/assets/map_*.lanelet.osm` と
       drift していないこと** を staleness gate として検証。これで「refresh script を回し
       忘れて summary 数値が古い」型の bug が CI で落ちる。
     - 既存 opt-in browser smoke (`tests/js/map_console_smoke.spec.mjs`) の Berlin Mitte
       dataset switch ケースに、Lanelet2 export カードの `#lanelet-count > 1000` /
       `#lanelet-reg-count > 0` / `#lanelet-reg-tags` に `lane_connection` chip があること
       を追加。
     - 両 Lanelet2 export パスが Autoware 風 `<MetaInfo format_version="1" map_version="1"
       projector_type="local" origin_lat origin_lon generator/>` を root 直下の最初に emit
       する形に変更。stock Lanelet2 reader は format/map_version を読む、Autoware Universe の
       map_loader は projector origin を `map_projector_info.yaml` の sidecar 無しでも復元できる。
  ...
  100. `_build_committed_osm_dataset()` を生やして Berlin と Tokyo Ginza を一本化。
       ...
       OSM regulatory nodes の自動 projection も dataset 横断で動く。
       Tokyo Ginza (139.7600,35.6680,139.7750,35.6750) を helper 経由で committed
       化: 548 nodes、19 turn restrictions、104 traffic_signals + 160 crossings +
       17 stops + 1 speed_camera、route `n472 → n141` (2 389 m / 38 edges)、
       500 m reachable overlay from n472、SRTM elevations付き。
       viewer の DATASET_URLS / ROUTE_URLS / REACHABLE_URLS / RESTRICTIONS_URLS /
       LANELET_URLS / dropdown / Lanelet2 download button label に Tokyo Ginza を
       追加。3 都市目の committed dataset なので map console で「世界を旅する」
       narrative が立つ。`validate-lanelet2-tags` ok。

  99. Berlin Mitte に turn_restrictions + demo route + reachable overlay を追加。
      `scripts/fetch_osm_turn_restrictions.py` で /tmp に Berlin TRs を fetch（37 raw）、
      `refresh_docs_assets.py` の Berlin block で `convert_osm_restrictions_to_graph`
      を通して 17 件 commit (`docs/assets/berlin_mitte_turn_restrictions.json`)。
      n32 → n327 (2 659 m, 48 edges) の TR-aware route を `route_berlin_mitte.geojson`、
      n32 から 600 m budget の reachable overlay を `reachable_berlin_mitte.geojson` に
      生成。`_write_paris_grid_reachability_asset` を `_write_reachability_asset(
      dataset_id, ...)` に refactor し、Paris は薄い wrapper 経由で呼ぶ。
      viewer の `ROUTE_URLS` / `REACHABLE_URLS` / `RESTRICTIONS_URLS` 全てに
      Berlin Mitte を追加して dataset 切替で自動 overlay 表示。

  98. per-lane lane_boundary subtype を outermost=solid / interior=dashed に分離。
      Autoware は boundary subtype から lane change 可否を読むので、すべて solid
      だと multi-lane lanelet 間で planner が lane change を許可しない。各 lane の
      左右 boundary に対し:
      `solid` if lane_idx == 0 or lane_idx == n_lanes-1 or n_lanes==1 → outermost、
      else `dashed` → interior（lane_markings が "solid" を示せば override）。
      Paris 配分: 2162 solid (outer) + 808 dashed (multi-lane interior 808 = 114·2 +
      91·4 + 28·6 + 6·8)。`validate-lanelet2-tags` 引き続き ok。

  97. `export_lanelet2_per_lane` も lane_connection regulatory_element を emit。
      `export_lanelet2` (single-lanelet 版) で既に出していた junction-incident
      lanelets を束ねる `subtype=lane_connection` regulatory_element を per-lane 版にも
      移植。`lanelet_id_by_edge` を起点に各 graph node に集まる lanelet を
      `from_start` / `from_end` ロールで列挙、junction_type / junction_hint を
      `roadgraph:` prefix tag に付与。Paris は 707 lane_connection, Berlin は
      1476 lane_connection を emit するようになり、Autoware planner が junction を
      跨いで lanelets を直列に辿れる。`validate-lanelet2-tags` 引き続き ok。

  96. OSM `highway=stop` / `give_way` の point を edge tangent から 4 m perpendicular line
      に合成して Lanelet2 stop_line way として emit。
      `_perpendicular_polyline()` で nearest segment への投影点 + 法線方向を計算し、
      `_osm_regulatory_observations` で `kind=stop_line` 観測に `polyline_m` を付与。
      既存の `_build_stop_line_way` がそれを `line_thin` way（subtype=solid）として emit
      （5 個 / Paris グリッド、`roadgraph:kind=stop_line` タグ付き）。

  95. SRTM elevation を committed Paris / Berlin に乗せる（Open-Elevation API 経由）。
      `scripts/fetch_node_elevations.py` が map GeoJSON の全 `kind=node` を batch で
      `api.open-elevation.com/api/v1/lookup` に投げて `{node_id: elevation_m}` を
      `/tmp/osm_real_data/<dataset>_node_elevations.json` に保存。
      `scripts/refresh_docs_assets.py` に `_apply_node_elevations()` を追加し、
      graph に elevation を stamp → node.attributes.elevation_m + 各 edge の
      polyline_z（endpoint elevations で線形補間）。`enrich_sd_to_hd` が自動的に
      `hd.slope_deg` を算出し、`export_lanelet2` / `export_lanelet2_per_lane` の
      graph-node emission は `<tag k="ele" v="N.NN"/>` を付与。Paris では
      855 node ele + 1081 slope_deg (|avg| 1.31°, |max| 55.56°)、Berlin では
      1883 node ele。`validate-lanelet2-tags` 引き続き `"result": "ok"` 0 errors。

  94. OSM `width=` タグを HD-lite envelope と Lanelet2 lanelet 属性に反映。
      `_collect_osm_way_polylines` に `width` tag を追加し、`_inject_osm_tags_into_graph_edges`
      が `edge.attributes.osm_width_m` に float 値で stamp する（87/1081 Paris edges カバー）。
      `_widen_hd_envelope_for_osm_lanes` は width → lanes → default の precedence で total
      road width を決めるよう refactor、`hd.quality` も `osm_width_tag` / `osm_lanes_offset_hd_lite`
      / `centerline_offset_hd_lite` の 3 階層で narrative 化。`_autoware_lanelet_tags_from_attributes`
      は `osm_width_m` があれば `<tag k="width" v="N.NN m"/>` を Lanelet2 relation に emit。
      commit 後 Paris distribution: 87 osm_width / 239 osm_lanes / 755 default。

  93. `export_lanelet2_per_lane` も semantic_rules から regulatory_element を emit。
      既存の `export_lanelet2`（single-lanelet 版）は camera_detections kwarg 経由で
      traffic_light regulatory_element / stop_line way を出していたが、per-lane 版は
      出していなかった。`lanelet_id_by_edge` を追跡し、各エッジの先頭 lanelet ID に対して
      `edge.attributes.hd.semantic_rules` を走査して `_build_traffic_light_regulatory` /
      `_build_stop_line_way` を呼ぶ（`world_xy_m` があるもののみ）。OSM 実 regulatory の
      `_osm_regulatory_observations` は `world_xy_m` を付与するようになり、Paris の 288
      traffic_signals が Lanelet2 OSM に 288 本の `type=regulatory_element, subtype=
      traffic_light` relation として出る。`apply_camera_detections_to_graph` の二重適用
      バグ（semantic_rules を 2 回 append して regulatory elements が 2x に膨らんでいた）
      を修正。
...
- **A4 Lanelet2 export inspector (v0.7.2.dev0, 2026-04-25 session):** refresh script で
  `_emit_lanelet_summary(lanelet_path, summary_path)` を 3 datasets ぶん回し、
  `docs/assets/map_<dataset>_lanelet_summary.json` に **lanelet 数 / regulatory_element 数 /
  per-subtype distribution / boundary subtype distribution / `ele` tag 数 / file size** を保存。
  `docs/map.html` のサイドバー右下にカードを追加して dataset 切替時に上記を rendering する。
  公開 OSM XML を parse せず軽い JSON だけで「Autoware に流せる Lanelet2 になっているか」が見える。
- **A5 directional `lane_connection` + Autoware MetaInfo (v0.7.2.dev0, 2026-04-26 session):**
  `lane_connection` regulatory_element が junction node の bundle 1 本ではなく、`(predecessor,
  successor)` の directed pair × N 本になり、`oneway=yes` / `oneway=-1` を解釈して flow が
  junction の出入りに合わせて roles が決まる（Autoware の planner が successor 関係を直接読める）。
  さらに root 直下に `<MetaInfo format_version="1" map_version="1" projector_type="local"
  origin_lat="…" origin_lon="…" generator="…">` を最初に emit するので、`map_projector_info.yaml`
  の sidecar を別途配らなくても projector frame を復元できる。Paris / Berlin / Tokyo の
...
- `docs/images/map_console_2d.png` / `docs/images/map_console_3d.png`: README / SHOWCASE 用の
  ...
  **2026-04-26 時点では Lanelet2 export inspector card が右側に追加されているが、現 hero PNG は
  card 追加前の世代。Codex が hero refresh を回す価値が高い（§5b 候補 B）。**
...
#### 2026-04-26 残し（Claude → Codex 引き継ぎ）

user は 5 件の候補を並べた中で 1〜3 を Claude に着手させ、残り 2 件は Codex に渡したい意向で
session を切った。**Codex は以下を上から順に拾うのが自然。**

A. **DONE 2026-04-28 Codex: 4 都市目の committed dataset を追加** — user は **San Francisco**（grid + 急坂で
   `node_ele_count` / `slope_deg` が映える）か **京都**（狭い古道で OSM `lanes=` /
   `width=` のグラデーションが試される）の 2 案を出した。Asia は Tokyo Ginza 済み、
   Europe は Paris / Berlin、Americas / 日本以外のアジア が未カバーなので **SF 推奨**。
   - 実装済み: San Francisco North Beach / Russian Hill bbox
     `-122.4300,37.7900,-122.4000,37.8100` を `sf_north_beach` として追加。
     committed surface は `map_sf_north_beach.geojson`,
     `map_sf_north_beach.lanelet.osm`,
     `map_sf_north_beach_lanelet_summary.json`,
     `sf_north_beach_origin.json`,
     `sf_north_beach_turn_restrictions.json`,
     `sf_north_beach_camera_detections.json`,
     `route_sf_north_beach.geojson`,
     `reachable_sf_north_beach.geojson`。
     1 800 graph nodes / 2 227 centerlines、127 mapped OSM TR、876 OSM-derived
     regulatory observations、1 800 SRTM elevation nodes、route `n1377 → n499`
     2.76 km / 52 edges、reachable `n1377` 600 m。Lanelet2 summary は
     3 458 lanelets / 7 475 regulatory_elements / 1 800 ele nodes、
     lane_connection 6 006。`validate-lanelet2-tags` は errors=0、
     warnings は missing speed_limit のみ。
   - viewer は `DATASET_URLS` / `ROUTE_URLS` / `REACHABLE_URLS` /
     `RESTRICTIONS_URLS` / `LANELET_URLS` / `LANELET_SUMMARY_URLS` /
     dropdown / Lanelet2 download label を更新済み。`tests/test_lanelet_summary.py`
     と `tests/test_attribution.py` も SF を含む。
   - 入口：`scripts/fetch_osm_highways.py` / `fetch_osm_turn_restrictions.py` /
     `fetch_osm_regulatory_nodes.py` / `fetch_node_elevations.py` を `/tmp/osm_real_data/`
     向けに走らせ、`scripts/refresh_docs_assets.py` の `_build_committed_osm_dataset(...)`
     呼び出し（berlin / tokyo 並びに sf を追加）と viewer 側の `DATASET_URLS` /
     `ROUTE_URLS` / `REACHABLE_URLS` / `RESTRICTIONS_URLS` / `LANELET_URLS` /
     `LANELET_SUMMARY_URLS` / dropdown / Lanelet2 download button label を更新。
   - bbox 候補（SF）: Embarcadero〜Russian Hill の急斜面が乗る `-122.43,37.79,-122.40,37.81`。
     Tokyo Ginza と同程度の規模で 500〜800 nodes が出ると地図 console の比較が読みやすい。
   - 注意:
     * `paris_grid_origin.json` が無い件は `paris_merged_origin.json` の `lat0/lon0` を
       読む helper パスを通っているので、SF の `*_origin.json` も `latitude/longitude` 形
       で書く（berlin / tokyo と同じ）。
     * raw `/tmp/...` を repo に commit しない（`.gitignore` / 既存 ATTRIBUTION ルールに従う）。
     * elevation cache は Open-Elevation API の rate limit に注意（fetch_node_elevations.py
       が 100 ノード batch で 1 秒 sleep を挟む）。
