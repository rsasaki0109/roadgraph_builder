...
- **state:** **v0.7.2.dev0 open on main** after **v0.7.1 shipped (2026-04-21)**。tag `v0.7.1` は `8282f7c`。
  ...
  最新 full local `pytest` = **644 passed / 3 skipped / 4 deselected**（opt-in marker 除外）。
- **直近の sessions (2026-04-21〜2026-04-22) で landed:**
  ...
  65. HMM bridge ambiguity benchmark。
      ...
      吸われず connected chain に残ることを count と unit test で固定。committed baseline は 0.496s。
...
- **Benchmarks (v0.5+):** `scripts/run_benchmarks.py` / `make bench` が
  ...
  （connected boundary HMM 500 samples + disconnected bridge distractors）/ `export_geojson_grid_120_compact`
...
- テスト: `pytest` で 644 passed / 3 skipped / 4 deselected が baseline。
...
- [ ] `pytest` が 644 passed / 3 skipped / 4 deselected で通ること（上下 ±1 は OK、大きく減ったら
      skip 理由を確認）。
