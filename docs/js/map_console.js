...
const DATASET_URLS = {
  ...
  sf_north_beach: "assets/map_sf_north_beach.geojson",
...
const ROUTE_URLS = {
  ...
  sf_north_beach: "assets/route_sf_north_beach.geojson",
};
const REACHABLE_URLS = {
  ...
  sf_north_beach: "assets/reachable_sf_north_beach.geojson",
};
...
const LANELET_URLS = {
  ...
  sf_north_beach: "assets/map_sf_north_beach.lanelet.osm",
};
// Per-dataset Lanelet2 summary JSON. Generated by refresh_docs_assets and
// fetched on dataset switch so the inspector can show subtype counts without
// the browser parsing a multi-megabyte XML.
const LANELET_SUMMARY_URLS = {
  paris_grid: "assets/map_paris_grid_lanelet_summary.json",
  berlin_mitte: "assets/map_berlin_mitte_lanelet_summary.json",
  tokyo_ginza: "assets/map_tokyo_ginza_lanelet_summary.json",
  sf_north_beach: "assets/map_sf_north_beach_lanelet_summary.json",
};
const RESTRICTIONS_URLS = {
  ...
  sf_north_beach: "assets/sf_north_beach_turn_restrictions.json",
};
...
function renderLaneletSummary(summary) {
  const card = document.getElementById("lanelet-card");
  if (!card) return;
  if (!summary) {
    card.hidden = true;
    return;
  }
  text("lanelet-count", formatCount(summary.lanelet_count));
  text("lanelet-reg-count", formatCount(summary.regulatory_element_count));
  text("lanelet-ele", formatCount(summary.node_ele_count));
  text("lanelet-size", formatBytesShort(summary.file_size_bytes));

  const regHost = document.getElementById("lanelet-reg-tags");
  if (regHost) {
    regHost.innerHTML = "";
    const subs = summary.regulatory_subtypes || {};
    for (const k of Object.keys(subs).sort()) {
      const tag = document.createElement("span");
      tag.className = "lanelet-tag " + k;
      tag.innerHTML = `${k} <b>${subs[k]}</b>`;
      regHost.appendChild(tag);
    }
  }
  const bndHost = document.getElementById("lanelet-bnd-tags");
  if (bndHost) {
    bndHost.innerHTML = "";
    const subs = summary.boundary_subtypes || {};
    for (const k of Object.keys(subs).sort()) {
      const tag = document.createElement("span");
      tag.className = "lanelet-tag " + k;
      tag.innerHTML = `${k} <b>${subs[k]}</b>`;
      bndHost.appendChild(tag);
    }
  }
  card.hidden = false;
}
...
async function show(which) {
  ...
  const laneletSummaryUrl = LANELET_SUMMARY_URLS[which];
  if (laneletSummaryUrl) {
    fetch(laneletSummaryUrl)
      .then((r) => (r.ok ? r.json() : null))
      .then((summary) => renderLaneletSummary(summary))
      .catch((err) => {
        console.warn("lanelet summary fetch failed:", err);
        renderLaneletSummary(null);
      });
  } else {
    renderLaneletSummary(null);
  }
