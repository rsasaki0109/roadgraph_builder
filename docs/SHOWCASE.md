...
![2D map console: Paris OSM grid with route, reachability, turn restrictions, and inspector metrics](images/map_console_2d.png)

![3D graph console: same Paris grid rendered as a Three.js preview with matching inspector metrics](images/map_console_3d.png)

![Route diagnostics comparison showing safe A* and the Paris Dijkstra fallback](images/route_diagnostics_compare.png)

Map-console PNGs are regenerated by `scripts/render_map_console_screenshot.py` and the animated GIF by `scripts/record_map_console_hero.py` (both drive system Chrome via Playwright and pull OSM tiles, Leaflet, and Three.js over the network; the GIF recorder additionally needs the Playwright ffmpeg binary — `npx playwright install ffmpeg`). The 3D view is also reachable directly at `map.html?view=3d`.
