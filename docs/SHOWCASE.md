...
Map-console PNGs are regenerated by `scripts/render_map_console_screenshot.py` (system Chrome via Playwright; pulls OSM tiles, Leaflet, and Three.js over the network). The 3D view is also reachable directly at `map.html?view=3d`.
