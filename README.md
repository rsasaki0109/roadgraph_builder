...
Open the interactive version locally: `cd docs && python3 -m http.server 8765`, then visit **`http://127.0.0.1:8765/map.html`** (pick a committed SD/HD sample from the dropdown — Paris grid, **Berlin Mitte**, GPS-derived Paris/Berlin, or the synthetic toy — with HD-lite lane boundaries drawn alongside every centerline and synthetic regulatory markers (traffic lights, stop lines, crosswalks, speed limits) on the Paris grid; the **Mode** select cycles **Basic → SD → HD → Full** so you can watch the same graph grow from centerlines only up to the full HD-lite view; add `?view=3d` for the 3D graph preview, `?dataset=berlin_mitte` to jump straight to Berlin, `?from=n312&to=n191` to restore the Paris turn-restriction-aware route — the 2D view auto-fits to the route, the inspector's **Route steps** card lists each edge / direction / cumulative length, `Download route GeoJSON` streams the current route as `route_<dataset>_<from>_<to>.geojson`, and the **Reach from click** toolbar lets you pick any node and get a live reachability overlay for the selected budget without leaving the page).
